...
## 標題
VIVERSE SDK Skills for AI IDEs：從平台整合到 Polygon Streaming 資產整合

## 一句話摘要
這個版本的 workshop 先用一條清楚的主線，展示如何把 standalone app 發布到 VIVERSE，並用 AI IDE skills 完成 auth、avatar 與 leaderboard 整合；接著再加上一段 Polygon Streaming SDK 延伸 demo，證明同一套 skill-based workflow 也能處理更高約束的 3D runtime asset integration。

## 版本定位
這不是一個全新的 workshop。

它是原本 workshop 的 extended version。

結構上仍然只有一條主線，再加一段進階延伸內容。

## 受眾定位
這個版本適合想看到兩種 skill 能力的人：

1. 平台能力整合，例如 publish、auth、avatar、leaderboard
2. 3D runtime 資產整合，例如 Polygon Streaming model replacement

## 核心論點
skill repository 的價值，不只是讓 AI 產生更快的程式碼。

它真正提供的是：

1. 正確的整合順序
2. 真實執行環境的規則
3. 容易出錯之處的 guardrails
4. 可以重播、可驗證的工程工作流程
...
2. publish baseline app 到 VIVERSE
...
1. 切換到一個適合展示 streamed asset replacement 的 app
...
作為一個 clean before-state，讓 publish、auth、avatar、leaderboard 的價值容易被看見。
...
## 第二段為什麼要另外選 App
PS SDK integration 最重要的是 streamed asset replacement 的可見差異。

因此第二段不要勉強塞進 hide-and-seek。
...
3. wrapper event 而不是 user callback 的成功判定方式
...
## 第二段的 Before And After
第二段要強調的前後差異是：

1. Before：app 仍使用原本的 procedural 或 placeholder aircraft
2. After：app 改用 streamed `.xrg` model，並且在正確的 runtime 規則下完成替換

這個差異會比口頭說明更有說服力。

## 整體 Lesson
這個 extended version 想讓觀眾看到兩層結論：
...
Slide 7：baseline world 與 upgraded world 對照
...
Slide 11：PS demo 的 before/after 對照
